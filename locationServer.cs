...
class Server
{
    ...
    public static string logPath = "LocationServerLog.txt";
...
class Update
{
    ...
    public static void UpdateDictionaryBackup()
    {
        //This creates a new stream writer
        StreamWriter streamWriter = new StreamWriter(Server.dictionaryBackupPath);
...
class Logging
{
    //This creates a new instance of the mutex class
    ...
    private void Log(string output, string ipAddress, string protocolStatus)
    {
        ...
        string logMessage = ipAddress + " - - " + DateTime.Now.ToString("'['dd'/'MM'/'yyyy':'HH':'mm':'ss zz00']'") + " \"" + Regex.Split(output, "\r\n")[0] + "\" " + protocolStatus;
        ...
        {
            //This prints to the screen an error message
            logMessage = "ERROR: Could not write to log at path: " + Server.logPath;
        }
