...
class Client
{
    ...
    private static string ipAddress = "whois.net.dcs.hull.ac.uk";
    ...
    //These are used to store the varables that will bepassed from the client to the server
    ...
    {
        //These are used to store varibales used to verify the argumensts given to the client
        int noOfArgs = 0;
        bool argBool = false;
        ...
        //If the argumesnts given are valid this calls the method which updates the server
        if (argBool)
        {
            ToServer(args, ref noOfArgs);
        }
    }
        ...
        if (args.Length != 0)
        {
            ...
            while (i < args.Length)
            {
                switch (args[i])
                {
                    ...
                    //This is triggured when the port is to be changed
                    case "-p":
                    ...
                    default:
                        ...
                        //This tests to see if a username has already been set
                        if (usernameBool == true)
                        {
                            ...
                            //If the location is already set that must mean that an invalid set of arguments has been entered as the last argument will always be the location thus this portion of code will exit the program under these circumastances
                        ...
                        else
                        {
                            ...
                            //This is a boolean to track the status of the username
                            usernameBool = true;
                        }
    ...
    private static void ToServer(string[] args, ref int noOfArgs)
    {
        ...
        //This ensures that a correct set of arguments has been recieved
        if ((args.Length - noOfArgs == 1) || (args.Length - noOfArgs == 2))
        {
            //If the number of arguments unnacounted for is one then there must only be arguments for a username
            ...
            //If the number of arguments unnacounted for is one then there must be arguments for a username and a location
            else
            {
                #region Two Arg
        ...
        try
        {
            ...
            //This ensures that if the server hangs for too long the client doens't wait for a responce
            ...
            //This will skip the responce code if the responce from the server is not needed
            if (serverOutput)
            {
                ...
                while ((client.Connected) && (!streamReader.EndOfStream) && (streamReader.Peek() != -1))
                {
                    ...
                    //This addes it to the variable to be printed to the screen
                ...
                //This is used to remove none essential lines from HTTP responces
            ...
            {
                //This ensures the client doesn't crash when the server is trying to force a responce that is unneeded
                while ((client.Connected) && (!streamReader.EndOfStream) && (streamReader.Peek() != -1))
                {
        ...
        //This prints the server responce to the screen
